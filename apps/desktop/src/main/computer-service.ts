...
import {
  ...
  requireStreamMetadata,
  validateShareSelection,
  waitFor,
  type ActiveCaptureSource,
  type CaptureCatalog,
  type CaptureRecord,
  type CursorPosition,
  type ShareSelection,
  type WindowMetadata,
} from "./computer-service-model.js";
...
export class DesktopComputerService {
  ...
  resizePictureInPicture(pinchDeltaY: number): void {
    ...
    const nextScale = nextComputerPreviewScale(this.previewScale, pinchDeltaY);
  ...
  private async resolveDesktopSource(
    ...
    const exact = sources.find((candidate) => candidate.id === source.sourceId);
    if (exact) return exact;
  ...
  private async loadCatalog(
    ...
    if (this.catalog && !force && Date.now() - this.catalogLoadedAt < 5_000) {
      return this.catalog;
    }
  ...
  private async ensureCaptureWindow(): Promise<BrowserWindow> {
    ...
    capture.webContents.session.setDisplayMediaRequestHandler(
      (request, callback) => {
        ...
        if (!pending || request.frame !== pending.webContents.mainFrame) {
          callback({});
          return;
        }
  ...
  private async ensurePreview(): Promise<BrowserWindow> {
    ...
    const minimumSize = this.layout
      ? scaledComputerPreviewSize(this.layout, COMPUTER_PREVIEW_MIN_SCALE)
      : { width: 108, height: 72 };
    const maximumSize = this.layout
      ? scaledComputerPreviewSize(this.layout, COMPUTER_PREVIEW_MAX_SCALE)
  ...
  private resizePreview(
    ...
    const previewSize = scaledComputerPreviewSize(layout, this.previewScale);
