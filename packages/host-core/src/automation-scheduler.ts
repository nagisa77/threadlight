...
export interface AutomationSchedulerOptions {
  now?: () => Date;
  intervalMs?: number;
  execute(automation: HostAutomation): Promise<AutomationExecutionResult>;
...
): { status: Exclude<HostAutomationRunStatus, "running">; summary: string } {
  if (result.error) {
