...
export class AutomationStore {
  ...
  snapshot(projectId: string): HostAutomationsSnapshot {
    return {
      projectId,
  ...
  all(): readonly HostAutomation[] {
    return this.read().automations;
  }

  get(id: string): HostAutomation | undefined {
    return this.read().automations.find((automation) => automation.id === id);
  }

  create(request: HostAutomationCreateRequest): HostAutomationsSnapshot {
  ...
  update(request: HostAutomationUpdateRequest): HostAutomationsSnapshot {
    validateAutomationInput(request);
    const stored = this.read();
  ...
  delete(projectId: string, id: string): HostAutomationsSnapshot {
    const stored = this.read();
    ...
    if (stored.automations.length === before) {
  ...
  due(at = this.now()): readonly HostAutomation[] {
  ...
  markCompleted(
    id: string,
    run: Pick<HostAutomationRun, "status" | "threadId" | "summary">,
  ): HostAutomation {
    const stored = this.read();
    const automation = stored.automations.find(
      (candidate) => candidate.id === id,
...
function isAutomation(value: unknown): value is HostAutomation {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const automation = value as Record<string, unknown>;
  try {
    normalizeSchedule(automation.schedule as HostAutomationSchedule);
  } catch {
    return false;
  }
  return (
...
function isAutomationRun(value: unknown): value is HostAutomationRun {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
