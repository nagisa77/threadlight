...
function decryptOptional(
  ...
  try {
    return nonEmpty(codec.decrypt(value));
  } catch {
    // Settings can be copied between hosts or opened by a headless Host after
    // being encrypted with Electron safeStorage. Fail closed for that secret
    // while keeping the settings surface available so it can be replaced.
    return undefined;
  }
}
