...
import { activateComposerMenuOnPointerDown } from "./controller.js";
...
export function ModelSelector({
  ...
  function togglePopover(fromKeyboard: boolean) {
    if (open) closePopover();
    else openPopover(fromKeyboard);
  }
  ...
  return (
    <>
      <button
        ...
        onPointerDown={(event) =>
          activateComposerMenuOnPointerDown(event, () => togglePopover(false))
        }
        onClick={(event) => {
          if (event.detail === 0) togglePopover(true);
        }}
        disabled={disabled}
